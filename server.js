...
app.delete('/messages/:id', async (req, res) => {
  ...
  try {
    ...
    const message = await db.query('SELECT * FROM messages WHERE id = ? AND (sender_id = ? OR recipient_id = ?)', [messageId, senderId, senderId]);
  ...
  db.query('SELECT * FROM users WHERE email = ?', [email], (err, results) => {
    ...
    db.query('UPDATE users SET reset_token = ?, reset_token_expiry = ? WHERE email = ?', [token, expiry, email], (err) => {
      ...
      const mailOptions = {
        ...
        text: `You requested a password reset. Click the link to reset your password: https://frontend-de6b-git-main-kondwanis-projects.vercel.app/ResetPassword?token=${token}`
      };
